...
@section('content')
    <div class="max-w-2xl mx-auto bg-white rounded-lg shadow overflow-hidden">
        <div class="px-6 py-4 border-b"><h2 class="text-xl font-bold text-gray-800">Add Menu Item</h2></div>
        <div class="p-6">
            <form action="{{ route('admin.menus.store') }}" method="POST">
                @csrf
                <div class="mb-4">
                    <label class="block text-gray-700 font-medium mb-2">Title <span class="text-red-500">*</span></label>
                    <input type="text" name="title" value="{{ old('title') }}" required placeholder="e.g. Home, About Us" class="w-full px-4 py-2 border rounded-lg focus:ring-2 focus:ring-[#084D3C]">
                    @error('title') <p class="text-red-500 text-sm mt-1">{{ $message }}</p> @enderror
                </div>
                <div class="mb-4">
                    <label class="block text-gray-700 font-medium mb-2">URL <span class="text-red-500">*</span></label>
                    <input type="text" name="url" value="{{ old('url') }}" required placeholder="e.g. / or #about or https://example.com" class="w-full px-4 py-2 border rounded-lg focus:ring-2 focus:ring-[#084D3C]">
                ...
                <div class="mb-4 space-y-3">
                    <label class="flex items-center gap-3 cursor-pointer">
                        <input type="checkbox" name="is_button" value="1" {{ old('is_button') ? 'checked' : '' }} class="w-5 h-5 rounded text-[#084D3C]">
                        <span class="text-gray-700">Style as a CTA Button (e.g. "Contact Us")</span>
                    </label>
                    <label class="flex items-center gap-3 cursor-pointer">
                        <input type="checkbox" name="is_active" value="1" {{ old('is_active', true) ? 'checked' : '' }} class="w-5 h-5 rounded text-[#084D3C]">
                ...
                <div class="flex justify-end gap-4">
                    ...
                    <button type="submit" class="px-4 py-2 bg-[#084D3C] text-white rounded-lg hover:bg-green-800">Add Item</button>
                </div>
            </form>
        </div>
    </div>
